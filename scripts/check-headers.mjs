// 전송 헤더 검사 — 결제 링크·판매 랜딩을 내보내는 페이지가 남의 iframe 안에 들어갈 수 있는가.
//
...
//
//   이 사이트가 특히 그렇다: /rankup 이 19,900원·29,900원짜리 결제 링크를 내보낸다.
//   그 페이지가 남의 iframe 에 담기면 투명 레이어로 덮어 다른 곳으로 결제를 유도할 수 있다.
//
...
//
// 사용: node scripts/check-headers.mjs   (실패하면 종료코드 1)
...
console.log("\n[클릭재킹] 결제 링크를 내보내는 페이지가 남의 iframe 에 들어가지 않는다");
{
  ...
  // ⚠️ CSP 는 frame-ancestors 만 담는다. script-src 등을 여기 넣으면 외부 스크립트가
